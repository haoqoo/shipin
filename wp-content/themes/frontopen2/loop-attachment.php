...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
				...
				<?php if ( ! empty( $post->post_parent ) ) : ?>
					<p class="page-title"><a href="<?php echo get_permalink( $post->post_parent ); ?>" title="<?php echo esc_attr( sprintf( __( 'Return to %s', 'frontopen' ), strip_tags( get_the_title( $post->post_parent ) ) ) ); ?>" rel="gallery"><?php
						/* translators: %s - title of parent post */
						printf( __( '<span class="meta-nav">&larr;</span> %s', 'frontopen' ), get_the_title( $post->post_parent ) );
					?></a></p>
				<?php endif; ?>
				...
				<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<div class="entry-meta">
						<?php
							printf( __( '<span class="%1$s">By</span> %2$s', 'frontopen' ),
						...
						<?php
							printf( __( '<span class="%1$s">Published</span> %2$s', 'frontopen' ),
							...
							if ( wp_attachment_is_image() ) {
								...
								printf( __( 'Full size is %s pixels', 'frontopen' ),
									sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
										wp_get_attachment_url(),
										esc_attr( __( 'Link to full-size image', 'frontopen' ) ),
										$metadata['width'],
										$metadata['height']
									)
								);
...
?>
						<p class="attachment"><a href="<?php echo $next_attachment_url; ?>" title="<?php the_title_attribute(); ?>" rel="attachment"><?php
							$attachment_width  = apply_filters( 'frontopen_attachment_size', 900 );
							$attachment_height = apply_filters( 'frontopen_attachment_height', 900 );
